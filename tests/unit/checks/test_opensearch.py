...
def test_from_dsn_rejects_non_str() -> None:
    with pytest.raises(TypeError, match="DSN must be str"):
        OpenSearchHealthCheck.from_dsn(None, name="OpenSearch")  # type: ignore[arg-type]
    with pytest.raises(TypeError, match="got 'bytes'"):
        OpenSearchHealthCheck.from_dsn(b"http://localhost:9200", name="OpenSearch")  # type: ignore[arg-type]
